"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var map_1 = require('../Models/map');
var MapComponent = (function () {
    function MapComponent() {
        this.selectedCell = new core_1.EventEmitter();
    }
    MapComponent.prototype.onSelect = function (x, y) {
        this.selectedCell.emit({ x: x, y: y });
    };
    MapComponent.prototype.showMapData = function () {
        console.log(this.mapData);
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', map_1.Map)
    ], MapComponent.prototype, "mapData", void 0);
    __decorate([
        core_1.Output(), 
        __metadata('design:type', core_1.EventEmitter)
    ], MapComponent.prototype, "selectedCell", void 0);
    MapComponent = __decorate([
        core_1.Component({
            selector: 'map',
            templateUrl: 'app/components/views/map.component.html',
            styleUrls: ['app/components/views/map.component.css']
        }), 
        __metadata('design:paramtypes', [])
    ], MapComponent);
    return MapComponent;
}());
exports.MapComponent = MapComponent;
//# sourceMappingURL=map.component.js.map